...
-- Pilot progression (2026-08-26): rank computed from lifetime stats already
-- tracked in airline_pilot_stats; thresholds live in Config.PilotRanks.
local function GetPilotRank(citizenid)
    -- delegates to the cached shared resolver in main.lua (single source of truth)
    return GetPilotRankShared(citizenid)
end

-- Client UI asks for its own rank (menu locks, preview badges)
lib.callback.register('dps-airlines:server:getPilotRank', function(source)
    local player = Bridge.GetPlayer(source)
    if not player then return nil end
    local r, s = GetPilotRankShared(player.identifier)
    return { rank = r.rank, name = r.name, payMultiplier = r.payMultiplier,
             hours = s and s.flight_hours or 0, flights = s and s.successful_flights or 0 }
end)
...
lib.callback.register('dps-airlines:server:startFlight', function(source, data)
    ...
    -- Rank gate: bigger aircraft need higher pilot rank
    local pilotRank = GetPilotRank(player.identifier)
    if (aircraft.minRank or 1) > pilotRank.rank then
        local need = Config.PilotRanks[aircraft.minRank or 1]
        return nil, ('Requires %s — you are %s'):format(need and need.name or 'a higher rank', pilotRank.name)
    end
...
lib.callback.register('dps-airlines:server:completeFlight', function(source, data)
    ...
    -- Captain's rank scales the whole crew pot (Config.PilotRanks.payMultiplier)
    local captainRank = GetPilotRank(flight.pilot_citizenid)
    totalPay = math.floor(totalPay * (captainRank.payMultiplier or 1.0))
    ...
    for _, crew in ipairs(crewRows or {}) do
        ...
        if crewPlayer then
            crewMembers[#crewMembers + 1] = {
                ...
                role = crew.role,
                -- carry the already-paid amount so DistributeCrewPay's double-pay
                -- guard (payments.lua) can skip attendants paid via attendantService
                pay_amount = crew.pay_amount,
            }
        end
    end
    ...
    -- Flight hours are credited from the SERVER-KNOWN distance at a reference
    -- cruise speed, NOT from client data.duration (which is uncapped and would
    -- let a pilot bank arbitrary hours per flight to reach rank/pay tiers).
    -- Same server-sourced philosophy as the night/weather/landing multipliers.
    local REFERENCE_CRUISE_MPS = 180.0  -- ~650 km/h, mid of the fleet
    local creditedHours = math.min(6.0, (flight.distance or 0) / REFERENCE_CRUISE_MPS / 3600.0)
    ...
    MySQL.update.await([[
        ...
        totalPay, creditedHours, player.identifier
    })
...
lib.callback.register('dps-airlines:server:spawnAircraft', function(source, model, spawnIndex, airportCode)
    ...
    local sp = Bridge.GetPlayer(source)
    if not sp then return nil end
    local spRank = GetPilotRank(sp.identifier)
    if (aircraft.minRank or 1) > spRank.rank then
        local need = Config.PilotRanks[aircraft.minRank or 1]
        return nil, ('Requires %s — you are %s'):format(need and need.name or 'a higher rank', spRank.name)
    end
